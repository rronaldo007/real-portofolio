...
- Media (user uploads, e.g. `SiteProfile.photo`) is stored under `data/media/` (the persistent disk, alongside SQLite) and served by Django via a `re_path` in `config/urls.py` — WhiteNoise only handles static. The À propos portrait + og:image come from `SiteProfile.photo_src` (uploaded `photo` → `photo_url` fallback); with neither set, no portrait/og:image is rendered.
...
- SEO: data-driven `<head>` partial (`templates/pages/_seo.html`: OG/Twitter/JSON-LD) + `/sitemap.xml` + `/robots.txt`, all fed from `SiteProfile`/`Project` via `pages.context_processors.site`. Full guide + Google Search Console state in `.claude/docs/seo.md`.
